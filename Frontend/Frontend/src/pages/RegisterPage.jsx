...
const RegisterPage = ({ setPage }) => {
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try {
      await apiFetch("/api/registr", { method: "POST", body: { login: form.login, fio: form.fio, email: form.email, password: form.password } });
      setPage("login");
  ...
  return (
    <div style={{ minHeight: "80vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: C.bg }}>
      ...
      <div style={{ background: C.white, borderRadius: 16, padding: "36px 40px", width: 360, boxShadow: "0 2px 12px rgba(0,0,0,0.07)" }}>
        ...
        {[
          { label: "Логин", k: "login" }, { label: "ФИО", k: "fio" },
          ...
          <div key={f.k} style={{ borderBottom: `1px solid ${C.border}`, marginBottom: 20 }}>
            <input value={form[f.k]} onChange={set(f.k)} type={f.type || "text"} placeholder={f.label}
        ...
        <Btn variant="ghost" onClick={() => setPage("login")} style={{ width: "100%", borderRadius: 24 }}>Войти</Btn>
      </div>
    </div>
  );
};